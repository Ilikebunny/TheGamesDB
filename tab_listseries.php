...
<?php endif; ?>
    ...
    if ($function == "Advanced Search") {
        ?>
            <a href="javascript: void();" onclick="$('#advancedSearchPanel').slideToggle(); if ($('#chevron').attr('src') == '<?= $baseurl; ?>/images/common/icons/expand_16.png') {
                        $('#chevron').attr('src', '<?= $baseurl; ?>/images/common/icons/collapse_16.png'); } else {
                        $('#chevron').attr('src', '<?= $baseurl; ?>/images/common/icons/expand_16.png');
                    }" style="text-decoration: none; outline: 0px; color: #EF5F00; font-weight: bold;">Advanced Search <img id="chevron" src="<?= $baseurl ?>/images/common/icons/collapse_16.png" alt="Expand/Collapse" style="vertical-align:middle;" /></a>
    ...
    ?>
                <a href="javascript: void();" onclick="$('#advancedSearchPanel').slideToggle(); if ($('#chevron').attr('src') == '<?= $baseurl; ?>/images/common/icons/expand_16.png') {
                            $('#chevron').attr('src', '<?= $baseurl; ?>/images/common/icons/collapse_16.png');
                        } else {
                            $('#chevron').attr('src', '<?= $baseurl; ?>/images/common/icons/expand_16.png');
                        }" style="text-decoration: none; outline: 0px; color: #EF5F00; font-weight: bold;">Advanced Search <img id="chevron" src="<?= $baseurl ?>/images/common/icons/expand_16.png" alt="Expand/Collapse" style="vertical-align:middle;" /></a>
...
while ($platformResult = mysql_fetch_assoc($platformQuery)) {
    ?>
                                        <option<?php if ($stringPlatform == $platformResult['id']) {
        echo " selected";
    } ?> value="<?php echo $platformResult['id']; ?>"><?php echo $platformResult['name']; ?></option>
            <?php
        }
        ?>
                                </select>
                        ...
                        <tr>
                            <td>Rating:
                                <select name="stringRating">
                                    <option value="">Any</option>
                                    <option<?php if ($stringRating == "eC - Early Childhood") {
            echo " selected";
        } ?>>EC - Early Childhood</option>
                                    <option<?php if ($stringRating == "E - Everyone") {
                    echo " selected";
                } ?>>E - Everyone</option>
                                    <option<?php if ($stringRating == "E10+ - Everyone 10+") {
                    echo " selected";
                } ?>>E10+ - Everyone 10+</option>
                                    <option<?php if ($stringRating == "T - Teen") {
                    echo " selected";
                } ?>>T - Teen</option>
                                    <option<?php if ($stringRating == "M - Mature") {
                    echo " selected";
                } ?>>M - Mature</option>
                                    <option<?php if ($stringRating == "RP - Rating Pending") {
                                        echo " selected";
                                    } ?>>RP - Pating Pending</option>
                                </select>
                            </td>
                            <td>Genre:
...
while ($genresResult = mysql_fetch_assoc($genresQuery)) {
    ?>
                                        <option<?php if ($stringGenres == $genresResult['genre']) {
        echo " selected";
    } ?> value="<?php echo $genresResult['genre']; ?>"><?php echo $genresResult['genre']; ?></option>
    <?php
}
?>
                                </select>
                        ...
                        <tr>
                            <td>
                                ...
                                <select name="stringCoop">
                                    <option value="">Any</option>
                                    <option<?php if ($stringCoop == "Yes") {
    echo " selected";
} ?>>Yes</option>
                                    <option<?php if ($stringCoop == "No") {
                                        echo " selected";
                                    } ?>>No</option>
        ...
        <form method="post" action="<?= $baseurl; ?>/search/" style="text-align: right;">
            ...
            <!--<p style="font-weight: bold;">Sort By: <select name="sortBy" onchange="this.form.submit();">
                    <option <?php if ($sortBy == "relevance") {
                                        echo "selected";
                                    } ?> value="relevance">Relevance</option>
                    <option <?php if ($sortBy == "GameTitle") {
                                        echo "selected";
                                    } ?> value="g.GameTitle">Name</option>
                    <option <?php if ($sortBy == "PlatformName") {
                                        echo "selected";
                                    } ?> value="p.name">Platform</option>
                    <option <?php if ($sortBy == "Genre") {
                                        echo "selected";
                                    } ?> value="g.Genre">Genre</option>
                    <option <?php if ($sortBy == "Rating") {
                                        echo "selected";
        ...
        ?>
                    ...
                    foreach ($elasticResults['hits']['hits'] as $game) {
                        ...
                        if ($searchview == "listing") {
                            ...
                            echo "<em><br />There is no overview available for this game.</em><br /><br />";
                                ...
                                <p style="font-size: 16px; color: #333;"><img src="<?= $baseurl ?>/images/common/consoles/png24/<?= $game->PlatformIcon ?>" alt="<?= $game->PlatformName ?>" style="vertical-align: -6px;" />&nbsp;<a style="color: #000;" href="<?= $baseurl; ?>/platform/<?php if (!empty($game->PlatformAlias)) {
                            echo $game->PlatformAlias;
                        } else {
                            echo $game->PlatformId;
                        } ?>/"><?= $game->PlatformName ?></a>&nbsp;|&nbsp;
                                    <?php
            ...
            if ($bannerResult != 0) {
                            ...
                            <div class="backgroundGradientGrey boxShadow" style="width: 356px; min-height: 102px; float: left; padding: 10px; margin: 10px; border-radius: 16px; border: 2px solid #333; background-color: #fff;">
                                ...
                                <h3 style="margin-top: 0px;"><a href="<?= $baseurl ?>/game/<?= $game->id ?>/" style="color: #000;"><?= $game->GameTitle ?></a></h3>
                                <p><img src="<?= $baseurl ?>/images/common/consoles/png24/<?= $game->PlatformIcon ?>" alt="<?= $game->PlatformName ?>" style="vertical-align: -6px;" />&nbsp;<a style="color: #000;" href="<?= $baseurl; ?>/platform/<?php if (!empty($game->PlatformAlias)) {
                            echo $game->PlatformAlias;
                        } else {
                            echo $game->PlatformId;
                        } ?>/"><?= $game->PlatformName ?></a></p>
                                <?php
                                $boxartQuery = mysql_query("SELECT keyvalue FROM banners WHERE banners.keyvalue = '$game->id' AND banners.filename LIKE '%front%' LIMIT 1");
                                $boxartResult = mysql_num_rows($boxartQuery);
            ...
            if ($banner->filename != "") {
                ?>
                            ...
                            <tr>
                                ...
                                <td class="<?php echo $class; ?>"><img src="<?= $baseurl ?>/images/common/consoles/png16/<?php echo $game->PlatformIcon; ?>" alt="<?php echo $game->PlatformName; ?>" style="vertical-align: middle;" /> <a style="color: #000;" href="<?= $baseurl; ?>/platform/<?php if (!empty($game->PlatformAlias)) {
                    echo $game->PlatformAlias;
                } else {
                    echo $game->PlatformId;
                } ?>/"><?= $game->PlatformName ?></a></td>
