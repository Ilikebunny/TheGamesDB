...
if ($loggedin = 1 && $adminuserlevel = 'ADMINISTRATOR') {
    if (isset($modID)) {
        ...
        if ($deletebanner = mysql_fetch_object(mysql_query($query))) {
            ...
            ## Delete files
            if (file_exists("../banners/$deletebanner->filename")) {
                unlink("../banners/$deletebanner->filename");
            }
            if (file_exists("../banners/_cache/$deletebanner->filename")) {
                unlink("../banners/_cache/$deletebanner->filename");
            }
            if (file_exists("../banners/_platformviewcache/$deletebanner->filename")) {
                unlink("../banners/_platformviewcache/$deletebanner->filename");
            }
            if (file_exists("../banners/_gameviewcache/$deletebanner->filename")) {
                unlink("../banners/_gameviewcache/$deletebanner->filename");
            }
            if (file_exists("../banners/_favcache/_banner-view/$deletebanner->filename")) {
                unlink("../banners/_favcache/_banner-view/$deletebanner->filename");
            }
            if (file_exists("../banners/_favcache/_boxart-view/$deletebanner->filename")) {
                unlink("../banners/_favcache/_boxart-view/$deletebanner->filename");
            }
            if (file_exists("../banners/_favcache/_tile-view/$deletebanner->filename")) {
        ...
        if (rename("../moderationqueue/$modItem->filename", "../banners/$filename")) {
            ...
            if (mysql_query("INSERT INTO banners (keytype, keyvalue, userid, dateadded, filename, languageid, resolution) VALUES ('$keytype', $modItem->gameID, $modItem->userID, $time, '$filename', 1, '$modItem->resolution')")) {
                ...
                if (mysql_query("DELETE FROM moderation_uploads WHERE id=$modItem->id")) {
                    ...
                    if (file_exists("../moderationqueue/_cache/$modItem->filename")) {
                        unlink("../moderationqueue/_cache/$modItem->filename");
                    }
